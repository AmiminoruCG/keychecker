...
from GLM import check_glm, pretty_print_glm_keys
from Moonshot import check_moonshot, pretty_print_moonshot_keys
...
if args.file:
    ...
    print('Enter API keys (OpenAI/Anthropic/AI21/MakerSuite/AWS/Azure/Mistral/Elevenlabs/Perplexity/GLM/Moonshot) one per line. Press Enter on a blank line to start validation')
...
async def validate_glm(key: APIKey, sem):
    async with sem, aiohttp.ClientSession() as session:
        IO.conditional_print(f"Checking GLM key: {key.api_key}", args.verbose)
        if await check_glm(key, session) is None:
            IO.conditional_print(f"Invalid GLM key: {key.api_key}", args.verbose)
            return
        IO.conditional_print(f"GLM key '{key.api_key}' is valid", args.verbose)
        api_keys.add(key)


async def validate_moonshot(key: APIKey, sem):
    async with sem, aiohttp.ClientSession() as session:
        IO.conditional_print(f"Checking Moonshot key: {key.api_key}", args.verbose)
        if await check_moonshot(key, session) is None:
            IO.conditional_print(f"Invalid Moonshot key: {key.api_key}", args.verbose)
            return
        IO.conditional_print(f"Moonshot key '{key.api_key}' is valid", args.verbose)
        api_keys.add(key)
...
glm_regex = re.compile(r'[a-f0-9]{32}\.[A-Za-z0-9_-]{16,64}')
moonshot_regex = re.compile(r'sk-[A-Za-z0-9]{48}')
...
async def validate_keys():
    ...
    for key in inputted_keys:
        ...
        elif "." in key and glm_regex.match(key):
            key_obj = APIKey(Provider.GLM, key)
            tasks.append(execute_with_retries(validate_glm, key_obj, concurrent_connections, 5))
        elif moonshot_regex.match(key):
            key_obj = APIKey(Provider.MOONSHOT, key)
            tasks.append(execute_with_retries(validate_moonshot, key_obj, concurrent_connections, 5))
...
def output_keys():
    ...
    for key in api_keys:
        ...
        elif key.provider == Provider.GLM:
            valid_glm_keys.append(key)
        elif key.provider == Provider.MOONSHOT:
            valid_moonshot_keys.append(key)
    ...
    if not args.proxyoutput:
        ...
        get_invalid_keys(valid_oai_keys, valid_anthropic_keys, valid_ai21_keys, valid_makersuite_keys, valid_aws_keys, valid_azure_keys, valid_vertexai_keys, valid_mistral_keys, valid_openrouter_keys, valid_elevenlabs_keys, valid_deepseek_keys, valid_xai_keys, valid_perplexity_keys, valid_glm_keys, valid_moonshot_keys)
        print()
        if valid_oai_keys:
        ...
        if valid_glm_keys:
            pretty_print_glm_keys(valid_glm_keys)
        if valid_moonshot_keys:
            pretty_print_moonshot_keys(valid_moonshot_keys)
    else:
        ...
        print("GLM_API_KEY=" + ','.join(key.api_key for key in valid_glm_keys if key.has_balance and not key.rate_limited))
        print("MOONSHOT_API_KEY=" + ','.join(key.api_key for key in valid_moonshot_keys if key.has_balance and key.kimi_k26_usable))
    if should_write:
